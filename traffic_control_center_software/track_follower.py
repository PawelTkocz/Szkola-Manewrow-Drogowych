...
from traffic_control_center_software.schemas import MovementInstruction, SpeedModifications
...
class TrackFollower():
    ...
    def get_movement_instruction(self, live_car_data: LiveCarData, track: Track, desired_end_state: ManoeuvrePhaseEndState) -> MovementInstruction:
    ...
    def turning_policy(self, live_car_data: LiveCarData, track: Track, desired_end_state: ManoeuvrePhaseEndState) -> Directions:
    ...
    def speed_policy(
        self, live_car_data: LiveCarData, track: Track, desired_end_state: ManoeuvrePhaseEndState
    ):
        ...
        for speed_modification in [
            ...
            will_go_off_track = self.will_go_off_track(car_simulation, track, desired_end_state)
    ...
    def meets_end_state_condition(self, car_simulation: CarSimulation, track: Track, end_state: ManoeuvrePhaseEndState) -> bool:
        closest_track_point_index = self.index_of_closest_track_point(car_simulation, track)
        goal_point_track_index = track.find_index_of_closest_point(end_state["front_middle_position"])
        start_state = car_simulation.get_current_state()
        result = False
        while closest_track_point_index <= goal_point_track_index:
            if car_simulation.velocity <= end_state["velocity"]:
                result = True
                break
            car_simulation.move(movement_decision={"turn_direction": self.turning_policy(car_simulation.get_live_data(), track, end_state), "speed_modification": SpeedModifications.BRAKE})
            closest_track_point_index = self.index_of_closest_track_point(car_simulation, track)
        car_simulation.restore_state(start_state)
        return result

    def will_go_off_track(self, car_simulation: CarSimulation, track: Track, desired_end_state: ManoeuvrePhaseEndState):
        """
        ...
        for _ in range(self.simulation_max_future_steps):
            car_simulation.move(movement_decision={"turn_direction": self.turning_policy(car_simulation.get_live_data(), track, desired_end_state), "speed_modification": SpeedModifications.BRAKE}) # was no change before
            if self.distance_to_track(car_simulation, track) > self.max_distance_to_track:
                went_off_track = True
                break
